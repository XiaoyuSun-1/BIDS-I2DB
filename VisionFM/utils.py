...
) # to ignore the warning when calculating metrics
...
def iou(output, target, smooth=1.0, threshold=0.5, return_ori=False):
    """
    IoU with smoothing. If return_ori is True, 
    return per image IoU [B]; else mean IoU (float).
    """
    # to numpy binary masks
    if isinstance(output, torch.Tensor):
    ...
    B = output.shape[0]
    if B == 0:
        return np.zeros((0,)) if return_ori else 0.0
...
def iou_mc(output, target, n_classes, smooth=1.0, return_ori=False):
    """
    Multi-class IoU with smoothing. If return_ori is True,
    return [B, C]; else mean per class [C].
    """
    # to numpy class indices
    if isinstance(output, torch.Tensor):
        output = output.argmax(dim=1).cpu().numpy()  # [B, H, W]
    if isinstance(target, torch.Tensor):
        target = target.cpu().numpy()                # [B, H, W]
    ...
    B = output.shape[0]
    if B == 0:
        return np.zeros((B, n_classes)) if return_ori else np.zeros((n_classes,))

    # for each image b and class c, compute IoU
    all_ious = np.zeros((B, n_classes), dtype=float)
    for b in range(B):
        for c in range(n_classes):
            p = (output[b] == c).astype(int).ravel()
            t = (target[b] == c).astype(int).ravel()
            inter = np.sum(p * t)
            union = np.sum(p) + np.sum(t) - inter
            all_ious[b, c] = (inter + smooth) / (union + smooth)

    if return_ori:
        return all_ious     
    return all_ious.mean(axis=0) 
    
     

def bacc(output, target, threshold=0.5, return_ori=False):
    """
    Binary balanced accuracy per image.
    If GT has no positives and prediction is all zeros, score is 1.0 else 0.0.
    """
    # to numpy binary masks
    if isinstance(output, torch.Tensor):
        output = (output.cpu().numpy() > threshold).astype(int)
    ...
    for b in range(B):
        ...
        t = target[b].ravel()
        if t.sum()==0:
            scores.append(1.0 if p.sum() == 0 else 0.0)
        else:
...
def bacc_mc(output, target, n_classes, threshold=0.5, return_ori=False):
    """
    Multiclass balanced accuracy per image and per class.
    If a class is absent in GT and prediction is also absent, score is 1.0 else 0.0.
    """
    # to numpy class indices
    if isinstance(output, torch.Tensor):
        output = output.argmax(dim=1).cpu().numpy()  # [B, H, W]
    ...
    B = output.shape[0]
    all_bacc = np.zeros((B, n_classes), dtype=float)
    ...
    for b in range(B):
        for c in range(n_classes):
            ...
            t = (target[b] == c).astype(int).ravel()
            if t.sum()==0:
                all_bacc[b,c] = 1.0 if p.sum() == 0 else 0.0
            else:
                all_bacc[b,c] = balanced_accuracy_score(t, p)

    if return_ori:
        return all_bacc    
    return all_bacc.mean(axis=0)
    
    

def acc(output, target, threshold=0.5, return_ori=False):
    """
    Binary pixel accuracy per image. Squeezes [B,1,H,W] to [B,H,W].
    If GT has no positives and prediction is all zeros, score is 1.0 else 0.0.
    """
    # to numpy binary masks
    if isinstance(output, torch.Tensor):
    ...
    for b in range(B):
        ...
        t = target[b].ravel()
        if t.sum()==0:
            accs.append(1.0 if p.sum() == 0 else 0.0)
        else:
...
def acc_mc(output, target, n_classes, return_ori=False):
    """
    Multiclass pixel accuracy per image and per class.
    If a class is absent in GT and prediction is also absent, score is 1.0 else 0.0.
    """
    # to numpy class indices
    if isinstance(output, torch.Tensor):
    ...
    B = output.shape[0]
    all_acc = np.zeros((B, n_classes), dtype=float)
    ...
    for b in range(B):
        for c in range(n_classes):
            ...
            t = (target[b] == c).astype(int).ravel()
            if t.sum()==0:
                all_acc[b, c] = 1.0 if p.sum() == 0 else 0.0
            else:
                all_acc[b,c] = accuracy_score(t, p)

    if return_ori:
        return all_acc      
    return all_acc.mean(axis=0)     

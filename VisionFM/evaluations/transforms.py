...
class NormalizeMinMax(object):
    def __call__(self, image, target=None):
        if not torch.is_tensor(image):
            image = tvF.to_tensor(image)
        # compute min/max over all pixels & channels
        mn = float(image.min())
        mx = float(image.max())
        diff = mx - mn
        if diff > 0:
            ...
            # just subtract min for flat images
            image = image - mn
        if target is None:
            return image
        return image, target
    
class MaskToClass(object):
    def __call__(self, image, target=None):
        if target is None:
            return image
        # 0 to 0, 85 to 1, 170 to 2, 255 to 3
        target = (target // 85).long()
        return image, target    
